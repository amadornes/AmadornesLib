...
public abstract class Tile implements ILocationW {
    ...
    /**
     * @return A type that identifies this tile and makes it different from the ones in the same mod and others (recommended format: modid.tilename)
     */
    ...
    public void sendUpdatePacket() {
        ...
        if (FMLCommonHandler.instance().getEffectiveSide().isServer()) {
            if (isMultiPart()) {
                ...
                NetworkHandler.sendToAll(new PacketTileUpdate(getTileEntity(), this));
            }
        }
    }
    ...
    public double getDistanceFrom(double x, double y, double z) {
        ...
        double d3 = getX() + 0.5D - x;
        double d4 = getY() + 0.5D - y;
        double d5 = getZ() + 0.5D - z;
    ...
    public static final AxisAlignedBB INFINITE_EXTENT_AABB = AxisAlignedBB.getBoundingBox(Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY,
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        ...
        if (type == Blocks.enchanting_table) {
            bb = AxisAlignedBB.getBoundingBox(getX(), getY(), getZ(), getX() + 1, getY() + 1, getZ() + 1);
        } else if (type == Blocks.chest || type == Blocks.trapped_chest) {
            bb = AxisAlignedBB.getBoundingBox(getX() - 1, getY(), getZ() - 1, getX() + 2, getY() + 2, getZ() + 2);
        } else if (type != null && type != Blocks.beacon) {
            AxisAlignedBB cbb = type.getCollisionBoundingBoxFromPool(getWorld(), getX(), getY(), getZ());
            if (cbb != null) {
                bb = cbb;
            }
        }
    ...
    public abstract void renderStatic(int pass);
    
    public abstract void renderDynamic(int pass, float frame);
    
    public abstract void renderItem(ItemRenderType type);
